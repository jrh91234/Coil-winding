...
window.renderCharts = function(data) {
    try {
         ...
         if (paretoView === 'symptom') {
             ...
             paretoBarLabel = 'NG ตามรุ่น (ชิ้น)';
         } else {
             const machineNames = Object.keys(data.machineData || {}).sort();
             if (paretoMachineSelector) {
                 const oldVal = paretoMachineSelector.value;
                 paretoMachineSelector.innerHTML = '<option value="">เลือกเครื่อง</option>' + machineNames.map(m => `<option value="${m}">${m}</option>`).join('');
                 selectedParetoMachine = (oldVal && machineNames.includes(oldVal)) ? oldVal : (machineNames[0] || '');
                 paretoMachineSelector.value = selectedParetoMachine;
                 paretoMachineSelector.classList.remove('hidden');
             } else {
                 selectedParetoMachine = machineNames[0] || '';
             }

             const mData = selectedParetoMachine ? data.machineData[selectedParetoMachine] : null;
             separated = window.separateSetupData((mData && mData.ngBreakdownPcs) ? mData.ngBreakdownPcs : {});
             hasSetup = separated.setup.some(v => v > 0);
             paretoBarLabel = selectedParetoMachine ? `Production (${selectedParetoMachine})` : 'Production (ชิ้น)';
         }
         ...
         if(ctxP) {
             ...
             if ((paretoView === 'symptom' || paretoView === 'machine') && hasSetup) {
                 paretoDatasets.push({
