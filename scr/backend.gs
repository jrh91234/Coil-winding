...
function doGet(e) {
  try {
    ...
    if (action === "GET_JOB_ORDERS") {
      ...
      let unlinkedSorting = null;
      try {
        const sortingAll = getSortingByJobOrder_();
        unlinkedSorting = sortingAll["__NO_JO__"] || null;
      } catch (sortErr) { unlinkedSorting = null; }
      return ContentService.createTextOutput(JSON.stringify({
        status: "success",
        jobOrders: jobs,
        unlinkedSorting: unlinkedSorting   // งานรอ Sort ที่ยังไม่ผูกเลข Job Order
...
function doPost(e) {
  ...
  // --- ส่วนที่ 2.6: ระบบบันทึกขยะ / วัสดุเหลือใช้ (Waste / Scrap Log) ---
  // port มาจาก Lug-Screw-management-system (WasteService.gs) → เก็บลง Google Sheet จริง
  if (action === "GET_WASTE_TYPES") {
    ...
    try {
      ...
      if (!sheet) {
        ...
        sheet.appendRow(["TypeID", "TypeName", "Active", "CreatedAt", "CreatedBy"]);
        const defaults = ["ขยะอันตราย", "ขยะรีไซเคิล", "ขยะอุตสาหกรรมไม่อันตราย", "อื่น ๆ"];
        const nowStr = Utilities.formatDate(new Date(), "GMT+7", "yyyy-MM-dd HH:mm:ss");
        const rowsToAdd = defaults.map(function(name) {
          return ["WT-" + Utilities.getUuid().substring(0, 8).toUpperCase(), name, true, nowStr, "System"];
        });
        sheet.getRange(2, 1, rowsToAdd.length, 5).setValues(rowsToAdd);
        SpreadsheetApp.flush();
      }
      const rows = sheet.getDataRange().getValues();
      const headers = rows[0].map(h => String(h).trim());
      const idCol = headers.indexOf("TypeID");
      const nameCol = headers.indexOf("TypeName");
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (!name) continue;
        types.push({ typeId: String(rows[i][idCol] || "").trim(), typeName: name });
      }
      return ContentService.createTextOutput(JSON.stringify({status: "success", data: types})).setMimeType(ContentService.MimeType.JSON);
    } catch (err) {
      return ContentService.createTextOutput(JSON.stringify({status: "error", message: err.toString()})).setMimeType(ContentService.MimeType.JSON);
    } finally {
      lock.releaseLock();
  ...
  if (action === "ADD_WASTE_TYPE") {
    ...
    try {
      ...
      const name = String(data.typeName || "").trim();
      if (!name) return ContentService.createTextOutput(JSON.stringify({status: "error", message: "กรุณากรอกชื่อชนิดขยะ"})).setMimeType(ContentService.MimeType.JSON);
      const rows = sheet.getDataRange().getValues();
      const headers = rows[0].map(h => String(h).trim());
      const nameCol = headers.indexOf("TypeName");
      const activeCol = headers.indexOf("Active");
      for (let i = 1; i < rows.length; i++) {
        const rawActive = activeCol === -1 ? "" : String(rows[i][activeCol]).trim().toLowerCase();
        const isActive = rawActive === "" || rawActive === "true" || rows[i][activeCol] === true;
        if (isActive && String(rows[i][nameCol] || "").trim().toLowerCase() === name.toLowerCase()) {
          return ContentService.createTextOutput(JSON.stringify({status: "error", message: "ชนิดขยะนี้มีอยู่แล้ว"})).setMimeType(ContentService.MimeType.JSON);
        }
      }
      const typeId = "WT-" + Utilities.getUuid().substring(0, 8).toUpperCase();
      sheet.appendRow([typeId, name, true, Utilities.formatDate(new Date(), "GMT+7", "yyyy-MM-dd HH:mm:ss"), data.recorder || "System"]);
      SpreadsheetApp.flush();
      logUserAction(data.recorder || "System", data.role || "User", "ADD_WASTE_TYPE", "เพิ่มชนิดขยะ: " + name);
      return ContentService.createTextOutput(JSON.stringify({status: "success", typeId: typeId, message: "เพิ่มชนิดขยะสำเร็จ"})).setMimeType(ContentService.MimeType.JSON);
    } catch (err) {
      return ContentService.createTextOutput(JSON.stringify({status: "error", message: err.toString()})).setMimeType(ContentService.MimeType.JSON);
    } finally {
      lock.releaseLock();
  ...
  if (action === "DELETE_WASTE_TYPE") {
    const lock = LockService.getScriptLock();
    try {
      lock.waitLock(10000);
      const sheet = ss.getSheetByName("WasteTypes");
      if (!sheet) return ContentService.createTextOutput(JSON.stringify({status: "error", message: "ไม่พบข้อมูลชนิดขยะ"})).setMimeType(ContentService.MimeType.JSON);
      const rows = sheet.getDataRange().getValues();
      const headers = rows[0].map(h => String(h).trim());
      const idCol = headers.indexOf("TypeID");
      const activeCol = headers.indexOf("Active");
      for (let i = 1; i < rows.length; i++) {
        if (String(rows[i][idCol] || "").trim() === String(data.typeId || "").trim() && String(data.typeId || "").trim() !== "") {
          if (activeCol !== -1) sheet.getRange(i + 1, activeCol + 1).setValue(false);
          else sheet.deleteRow(i + 1);
          SpreadsheetApp.flush();
          logUserAction(data.recorder || "System", data.role || "User", "DELETE_WASTE_TYPE", "ลบชนิดขยะ: " + data.typeId);
          return ContentService.createTextOutput(JSON.stringify({status: "success", message: "ลบชนิดขยะสำเร็จ"})).setMimeType(ContentService.MimeType.JSON);
        }
      }
      return ContentService.createTextOutput(JSON.stringify({status: "error", message: "ไม่พบชนิดขยะที่ต้องการลบ"})).setMimeType(ContentService.MimeType.JSON);
    } catch (err) {
      return ContentService.createTextOutput(JSON.stringify({status: "error", message: err.toString()})).setMimeType(ContentService.MimeType.JSON);
    } finally {
      lock.releaseLock();
  ...
  if (action === "SUBMIT_WASTE") {
    ...
    if (!data.weightKg || isNaN(weight) || weight <= 0) {
      return ContentService.createTextOutput(JSON.stringify({status: "error", message: "กรุณากรอกน้ำหนักที่ถูกต้อง"})).setMimeType(ContentService.MimeType.JSON);
    }
    const lock = LockService.getScriptLock();
    try {
      lock.waitLock(10000);
      ...
      if (recordedAt && recordedAt.indexOf("T") !== -1 && recordedAt.indexOf("+") === -1 && recordedAt.indexOf("Z") === -1) {
        recordedAt += "+07:00";
      }
      ...
      SpreadsheetApp.flush();
      logUserAction(data.username || data.recorder || "System", data.role || "User", "SUBMIT_WASTE", "บันทึกขยะ: " + wasteId + " | " + data.wasteType + " | " + weight + " kg");
      return ContentService.createTextOutput(JSON.stringify({status: "success", wasteId: wasteId, message: "บันทึกข้อมูลขยะเรียบร้อย: " + wasteId})).setMimeType(ContentService.MimeType.JSON);
    } catch (err) {
      return ContentService.createTextOutput(JSON.stringify({status: "error", message: err.toString()})).setMimeType(ContentService.MimeType.JSON);
    } finally {
      lock.releaseLock();
  ...
  if (action === "GET_TODAY_WASTE" || action === "GET_WASTE_HISTORY") {
    ...
    const isToday = action === "GET_TODAY_WASTE";
    const dateFrom = String(data.dateFrom || "").trim();
    const dateTo = String(data.dateTo || "").trim();
    const filterType = String(data.wasteType || "").trim();
    const results = [];
    for (let i = 1; i < rows.length; i++) {
      const obj = {};
      ...
      if (!obj.WasteID) continue;
      const rowDate = String(obj.Date || "").substring(0, 10);
      if (isToday) {
        if (rowDate !== today) continue;
      } else {
        if (dateFrom && rowDate < dateFrom) continue;
        if (dateTo && rowDate > dateTo) continue;
        if (filterType && String(obj.WasteType || "").trim() !== filterType) continue;
      }
      results.push(obj);
    }
    results.sort(function(a, b) { return String(b.Timestamp || "").localeCompare(String(a.Timestamp || "")); });
          ...
          if (foundRow > -1) {
              ...
              if (finalStatus === "Completed") {
                  ...
                  try {
                      ...
                      const sortJobOrderCol = getCol("Job_Order");
                      const sortJobOrder = sortJobOrderCol !== -1 ? String(sortRow[sortJobOrderCol] || "").trim() : "";
...
// แยกแถวปกติ (ยอดจากเครื่อง) ออกจากแถวที่ระบบสร้างหลัง Sort เสร็จ (Batch_ID = "SORT-...")
// เพื่อไม่ให้ยอด NG ถูกนับซ้ำ (NG หลัง Sort เป็นส่วนหนึ่งของ NG ที่บันทึกไว้ตอนผลิตอยู่แล้ว)
function getProducedByJobOrder_() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (!out[jo]) out[jo] = {
      fg: 0, ngKg: 0, ngPcs: 0,
      fgProd: 0, ngKgProd: 0, ngPcsProd: 0,   // ยอดจากเครื่อง (ก่อน Sort)
      fgSort: 0, ngKgSort: 0, ngPcsSort: 0,   // ยอดที่เกิดจากผล Sort (แถว SORT-)
      machines: {}, firstDate: "", lastDate: ""
    };
    ...
    const batchId = batchCol !== -1 ? String(data[i][batchCol] || "").trim() : "";
    const isSortRow = batchId.indexOf("SORT-") === 0;
    ...
    if (isSortRow) {
      out[jo].fgSort += fg;
      out[jo].ngKgSort += ngKg;
      out[jo].ngPcsSort += ngPcs;
    } else {
      out[jo].fgProd += fg;
      out[jo].ngKgProd += ngKg;
      out[jo].ngPcsProd += ngPcs;
    }
    if (macCol !== -1) {
...
// รวมยอดงาน Sorting แยกตามเลข Job Order
// - Pending / Rejected = งานรอคัด (ยังไม่ตัดสินว่าเป็น FG หรือ NG)
// - Wait QC            = คัดแล้ว รอ QC อนุมัติ (ยังไม่ถูกโพสต์เข้า Production_Data)
// - Completed          = QC อนุมัติแล้ว → ระบบสร้างแถว SORT- ใน Production_Data ไปแล้ว
//                        จึงเก็บไว้ดูอย่างเดียว ห้ามเอาไปบวกกับยอดผลิต (กันนับซ้ำ)
// กันข้อมูลซ้ำ: 1 Job_ID นับได้ครั้งเดียว (ชีตอาจมีแถวซ้ำจากการแก้ไข/ตีกลับ)
function getSortingByJobOrder_(opts) {
  opts = opts || {};
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName("Sorting_Data");
  const out = {};
  ...
  const joCol = idx("job_order");
  if (joCol === -1) return out;
  const jobIdCol = idx("job_id");
  const statusCol = idx("status");
  const qtyCol = idx("qty");
  const prodCol = idx("product");
  const sympCol = idx("symptom");
  ...
  const toPcs = (raw, productName) => {
    const txt = String(raw || "").trim();
    const val = parseFloat(txt) || 0;
    if (val <= 0) return 0;
    return txt.toLowerCase().indexOf("kg") !== -1 ? getPcsFromKg(productName, val) : Math.round(val);
  };

  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    // งานที่ยังไม่ผูกเลข Job Order เก็บรวมไว้ในกลุ่ม __NO_JO__ เพื่อให้เห็นว่ามียอดค้างอยู่เท่าไร
    const jo = String(row[joCol] || "").trim() || "__NO_JO__";

    const status = statusCol !== -1 ? String(row[statusCol] || "").trim() : "";
    if (!status) continue;

    if (dateCol !== -1 && opts.start && opts.end) {
      const iso = getSheetDateISO(row[dateCol]);
      if (iso && (iso < opts.start || iso > opts.end)) continue;
    }

    const jobId = jobIdCol !== -1 ? String(row[jobIdCol] || "").trim() : "";
    if (jobId) {
      if (seenJobIds[jobId]) continue;
      seenJobIds[jobId] = true;
    }

    const productStr = prodCol !== -1 ? String(row[prodCol] || "").trim() : "";
    const pParts = productStr.split(" : ");
    ...
    if (status === "Pending" || status === "Rejected") {
      ...
      agg.completedFgPcs += toPcs(fgCol !== -1 ? row[fgCol] : 0, prodName);
      agg.completedNgPcs += toPcs(ngCol !== -1 ? row[ngCol] : 0, prodName);
      agg.completedJobs++;
    }
  }
  return out;
...
function getJobOrders_(opts) {
  ...
  const sorting = getSortingByJobOrder_();   // ยอดงานรอ Sort / รอ QC แยกตาม Job Order
  ...
  rows.forEach((row, i) => {
    ...
    const act = produced[jobOrder] || {
      fg: 0, ngKg: 0, ngPcs: 0,
      fgProd: 0, ngKgProd: 0, ngPcsProd: 0,
      fgSort: 0, ngKgSort: 0, ngPcsSort: 0,
      machines: {}, firstDate: "", lastDate: ""
    };
...
function getAdvancedDashboardData(reqStart, reqEnd, reqShift, reqType) {
  ...
  const result = {
    ...
    jobOrderSortData: {}, // ยอดงานรอ Sort / รอ QC ของแต่ละ Job Order ในช่วงที่เลือก
      ...
      if (rowDateStr >= startDate && rowDateStr <= endDate) {
        ...
        if (jobOrder) {
          if (!result.jobOrderData[jobOrder]) {
            result.jobOrderData[jobOrder] = {
              fg: 0, ngPcs: 0, ngKg: 0,
              fgProd: 0, ngPcsProd: 0,   // ยอดจากเครื่อง
              fgSort: 0, ngPcsSort: 0,   // ยอดที่มาจากผล Sort (แถว SORT-)
              ngNetPcs: 0, accountedPcs: 0,
              products: {}, machines: {}, dates: {}
            };
          }
          ...
          jd.ngKg += ngKg;
          jd.ngPcs += ngPcs;
          if (isSortPostedRow) { jd.fgSort += fg; jd.ngPcsSort += ngPcs; }
          else { jd.fgProd += fg; jd.ngPcsProd += ngPcs; }
  ...
  // ยอดงาน Sorting ของแต่ละ Job Order เฉพาะช่วงวันที่ที่เลือก
  try {
    result.jobOrderSortData = getSortingByJobOrder_({ start: startDate, end: endDate });
  } catch (e) { result.jobOrderSortData = {}; }
